...
this.onmessage = function (e) {
    if (e.data.addThis !== undefined) {
        ...
        console.log("Worker received inputCharacters: " + e.data.addThis.inputCharacters + " | shortestWord:" + shortestWord + " | longestWord:" + longestWord);
    }
...
function gridSolver(itt) {
    ...
    console.log(inputCharacters + " - " + inputCharacters.length);

    // initialize Grid array
    fill2DimensionsArray(grid, (inputCharacters.length * 2) - 1, (inputCharacters.length * 2) - 1);

    // FIRST WORD PLACEMENT


    var xhr = new XMLHttpRequest();
    xhr.open("GET", "../api/api.php?q=" + inputCharacters + "&offset=" + itt + "&shortestWord=" + shortestWord + "&longestWord=" + longestWord, false);  // synchronous request
    xhr.send(null);
    ...
    this.postMessage({result: "<br>Iteration <b>" + itt + "</b>. Starting with word: <b>" + inputCharacters + "</b><br><br>"});
...
function addWordToGrid(selectedItem) {
    ...
    try {
                        ...
                        if (finished === false) {
                            ...
                            if (topToBottomOccupied === false) {
                                ...
                                for (var scanYY = (yy - commonLetterFirstPosition); scanYY < (yy + word.length); ++scanYY) {
                                    grid[xx][scanYY] = word.charAt(pos);
                                    pos++;
                                }
                                console.log("Successfully Placed Word: " + foundWordsArray[selectedItem].word + " on the grid top to bottom!");


                                //this.postMessage({result: "Successfully Placed Word: " + foundWordsArray[selectedItem].word + " on the grid top to bottom!"});
                        ...
                        if (finished === false) {
                            ...
                            if (leftToRightOccupied === false) {
                                ...
                                for (var scanXX = (xx - commonLetterFirstPosition); scanXX < (xx + word.length); ++scanXX) {
                                    grid[scanXX][yy] = word.charAt(pos);
                                    pos++;
                                }
                                console.log("Successfully Placed Word: " + foundWordsArray[selectedItem].word + " on the grid left to right!");
                                // this.postMessage({result: "Successfully Placed Word: " + foundWordsArray[selectedItem].word + " on the grid left to right!"});
        ...
        if (counterRepeater > maxIterations) {
            this.postMessage({result: "Sorry, but solution cannot be found in first " + maxIterations + " iterations. Try changing the settings!"});
            this.postMessage({grid: "."});
            this.postMessage({buttonName: "Solve"});
            this.postMessage({end: "end"});
        } else {
    ...
    for (var y = 0; y < (grid[0].length) - 1; ++y) {
        for (var x = 0; x < (grid[0].length) - 1; ++x) {
            if (grid[x][y]) {
                ...
                var xhr = new XMLHttpRequest();
                xhr.open("GET", "../api/api.php?q=" + (remainingCharacters + letterFromBoard) + "&offset=" + 0 + "&shortestWord=" + shortestWord + "&longestWord=" + longestWord, false);  // synchronous request
                xhr.send(null);
...
function placeFirstWord(inputCharacters) {
    ...
    for (var c = 0; c < firstWordSplit.length; ++c) {
        grid[((inputCharacters.length - 1) + c)][(inputCharacters.length - 1)] = firstWordSplit[c];
    }
}
...
function showGrid(xs, ys) {
    ...
    for (var y = 0; y < (ys * 2) - 1; ++y) {
        gridTable += '<tr>';
        var gridTable2 = "";
        var itemsInRow = false;
        for (var x = 0; x < (xs * 2) - 1; ++x) {
            if (grid[x][y]) {
